import{n as e}from"./chunk-BVpWp9h4.js";import{t}from"./card-DCcP7Xt5.js";import{n,t as r}from"./squizStoryRenderer-4zIdV6yt.js";import{n as i,r as a,t as o}from"./xss-Cvy76lHc.js";import{n as s,t as c}from"./squizInfo.mock-x2sHGyES.js";function l(e){return!e||typeof e!=`object`?null:`data`in e?e.data??null:e}async function u(e,t){if(!e||typeof t?.resolveUri!=`function`)return null;try{return l(await t.resolveUri(e))}catch{return null}}function d(e){let t=/^matrix-asset:\/\/([a-zA-Z0-9.-]+)\/\d+(?::.+)?$/.exec(String(e??``));return t?t[1]:null}function ee(e){let t=/^matrix-asset:\/\/[a-zA-Z0-9.-]+\/(\d+)(?::.+)?$/.exec(String(e??``));return t?t[1]:null}async function te(e){let t=d(e),n=ee(e);if(!t||!n)return[];let r=`${S}?${C}=${encodeURIComponent(n)}`,i;try{let e=await fetch(r);if(!e.ok)return[];i=await e.json()}catch{return[]}return Array.isArray(i)?i.map(e=>e?.id).filter(Boolean).map(e=>`matrix-asset://${t}/${e}`):[]}async function f(e,t,n){let r=p(e.metadata||{},`content-cardImagePhoto`);if(r){let e=/^matrix-asset:\/\//.test(r)?r:t?`matrix-asset://${t}/${r}`:null,i=e?await u(e,n):null,a=i?.url||(Array.isArray(i?.urls)?i.urls[0]:``)||``;if(a)return{url:a,alt:i.name||``,width:null,height:null}}let i=e.thumbnail;return i?{url:i.url||``,alt:i.alt||i.title||``,width:i.width||null,height:i.height||null}:null}function p(e,t){let n=e?.[t];return Array.isArray(n)&&n[0]||``}async function ne(e,t,n){if(!e)return null;let r=e.metadata||{};return{title:p(r,`content-cardTitle`)||e.name||``,description:p(r,`content-cardDescription`),image:await f(e,t,n),date:p(r,`Date`),tagLabel:p(r,`Category`),href:e.url||``,actionText:p(r,`content-cardButtonText`)||g}}async function re(e,t){let n=Array.isArray(e)?e:[];return(await Promise.all(n.map(async e=>{let n=await ne(await u(e,t),d(e),t);return n?[n]:[]}))).flat()}function ie(e){let t=e.image,n=`card-title-${Math.random().toString(36).slice(2,9)}`,r=t?`
    ...
    </div>`;return e.href?`<a class="card" href="${i(e.href)}" aria-labelledby="${n}">${l}</a>`:`<article class="card">${l}</article>`}async function ae(e,t){let n=Array.isArray(e)?e:[];return(await Promise.all(n.map(async e=>{let n=await u(e,t);return n?{icon:p(n.metadata||{},`content-cardIcon`),label:n.name||``,href:n.url||``}:null}))).filter(Boolean)}function oe(e){if(!e)return null;let t=e.metadata||{};return{icon:p(t,`content-cardIcon`),title:p(t,`content-cardTitle`)||e.name||``,description:p(t,`content-cardDescription`),href:e.url||``}}async function se(e,t){let n=Array.isArray(e)?e:[];return(await Promise.all(n.map(async e=>{let n=oe(await u(e,t));return n?[n]:[]}))).flat()}function ce(e){let t=`homepage-card-title-${Math.random().toString(36).slice(2,9)}`,n=e.icon?`<i class="fal fa-lg fa-${o(e.icon)}" aria-hidden="true"></i>`:``,r=e.description?`<p class="homepage-card__description">${o(e.description)}</p>`:``,a=`
    <h3 class="homepage-card__title" id="${t}">${n}${o(e.title)}</h3>
    <div class="homepage-card__divider" aria-hidden="true"></div>
    ${r}`;return e.href?`<a class="homepage-card" href="${i(e.href)}" aria-labelledby="${t}">${a}</a>`:`<div class="homepage-card">${a}</div>`}function le(e){let{icon:t=``,label:n=``,href:r=``}=e||{},a=`
    <span class="mini-card__icon" aria-hidden="true">${t?`<i class="fa-thin fa-${o(t)}"></i>`:``}</span>
    <span class="mini-card__label">${o(n)}</span>`;return r?`<a class="mini-card" href="${i(r)}">${a}</a>`:`<div class="mini-card">${a}</div>`}async function ue(e,t,n){if(!e)return null;let r=e.metadata||{};return{title:p(r,`content-cardTitle`)||e.name||``,description:p(r,`content-cardDescription`),image:await f(e,t,n),date:p(r,`news-datePublished`),href:e.url||``}}function m(e){let t=Date.parse(e||``);return Number.isNaN(t)?-1/0:t}async function de(e,t){let n=Array.isArray(e)?e:[];return(await Promise.all(n.map(async e=>{let n=await ue(await u(e,t),d(e),t);return n?[n]:[]}))).flat().sort((e,t)=>m(t.date)-m(e.date))}function fe(e){let t=e.image,n=`news-card-title-${Math.random().toString(36).slice(2,9)}`,r=t?`
    <div class="card__image">
      <img src="${i(t.url)}" alt="${o(t.alt)}"${t.width?` width="${t.width}"`:``}${t.height?` height="${t.height}"`:``} />
    ...
    </div>`;return e.href?`<a class="card" href="${i(e.href)}" aria-labelledby="${n}">${s}</a>`:`<article class="card">${s}</article>`}function pe(e){return`
...
}`,...$.parameters?.docs?.source}}},ge=[`SinglePage`,`MultiplePages`,`Unresolvable`,`HomepageCards`,`MiniCards`,`MiniCardsNoLink`,`NewsCards`]}))();export{W as HomepageCards,q as MiniCards,X as MiniCardsNoLink,R as MultiplePages,$ as NewsCards,F as SinglePage,V as Unresolvable,ge as __namedExportsOrder,M as default};
//# sourceMappingURL=card.stories-fhGiK8tD.js.map