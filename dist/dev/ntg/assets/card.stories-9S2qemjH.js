import{n as e}from"./chunk-BVpWp9h4.js";import{t}from"./card-DCcP7Xt5.js";import{r as n,t as r}from"./component.helper-D3WXhhhy.js";import{n as i,t as a}from"./squizStoryRenderer-4zIdV6yt.js";import{n as o,r as s,t as c}from"./xss-Cvy76lHc.js";import{n as l,t as ee}from"./squizInfo.mock-x2sHGyES.js";function te(e){return!e||typeof e!=`object`?null:`data`in e?e.data??null:e}async function u(e,t){if(!e||typeof t?.resolveUri!=`function`)return null;try{return te(await t.resolveUri(e))}catch{return null}}function d(e){let t=/^matrix-asset:\/\/([a-zA-Z0-9.-]+)\/\d+(?::.+)?$/.exec(String(e??``));return t?t[1]:null}function ne(e){let t=/^matrix-asset:\/\/[a-zA-Z0-9.-]+\/(\d+)(?::.+)?$/.exec(String(e??``));return t?t[1]:null}async function re(e){let t=d(e),n=ne(e);if(!t||!n)return[];let r=`${C}?${w}=${encodeURIComponent(n)}`,i;try{let e=await fetch(r);if(!e.ok)return[];i=await e.json()}catch{return[]}return Array.isArray(i)?i.map(e=>e?.id).filter(Boolean).map(e=>`matrix-asset://${t}/${e}`):[]}async function f(e,t,n){let r=p(e.metadata||{},`content-cardImagePhoto`);if(r){let e=/^matrix-asset:\/\//.test(r)?r:t?`matrix-asset://${t}/${r}`:null,i=e?await u(e,n):null,a=i?.url||(Array.isArray(i?.urls)?i.urls[0]:``)||``;if(a)return{url:a,alt:i.name||``,width:null,height:null}}let i=e.thumbnail;return i?{url:i.url||``,alt:i.alt||i.title||``,width:i.width||null,height:i.height||null}:null}function p(e,t){let n=e?.[t];return Array.isArray(n)&&n[0]||``}async function ie(e,t,n){if(!e)return null;let r=e.metadata||{};return{title:p(r,`content-cardTitle`)||e.name||``,description:p(r,`content-cardDescription`),image:await f(e,t,n),date:p(r,`Date`),tagLabel:p(r,`Category`),href:e.url||``,actionText:p(r,`content-cardButtonText`)||g}}async function ae(e,t){let n=Array.isArray(e)?e:[];return(await Promise.all(n.map(async e=>{let n=await ie(await u(e,t),d(e),t);return n?[n]:[]}))).flat()}function oe(e){let t=e.image,n=`card-title-${Math.random().toString(36).slice(2,9)}`,r=t?`
    <div class="card__image">
      <img src="${o(t.url)}" alt="${c(t.alt)}"${t.width?` width="${t.width}"`:``}${t.height?` height="${t.height}"`:``} />
    </div>`:``,i=e.tagLabel||e.date?`
      <div class="card__header">
        ${e.tagLabel?`<span class="card__tag">${c(e.tagLabel)}</span>`:``}
        ${e.date?`<span class="card__date">${c(e.date)}</span>`:``}
      </div>`:``,a=e.description?`<p class="card__description">${c(e.description)}</p>`:``,s=e.actionText?`
      <div class="card__footer">
        <span class="card__action">${c(e.actionText)} <span class="card__action-icon">${h}</span></span>
      </div>`:``,l=`${r}
    <div class="card__content">${i}
      <div class="card__body">
        <h3 class="card__title" id="${n}">${c(e.title)}</h3>
        ${a}
      </div>${s}
    </div>`;return e.href?`<a class="card" href="${o(e.href)}" aria-labelledby="${n}">${l}</a>`:`<article class="card">${l}</article>`}async function se(e,t){let n=Array.isArray(e)?e:[];return(await Promise.all(n.map(async e=>{let n=await u(e,t);return n?{icon:p(n.metadata||{},`content-cardIcon`),label:n.name||``,href:n.url||``}:null}))).filter(Boolean)}function ce(e){if(!e)return null;let t=e.metadata||{};return{icon:p(t,`content-cardIcon`),title:p(t,`content-cardTitle`)||e.name||``,description:p(t,`content-cardDescription`),href:e.url||``}}async function le(e,t){let n=Array.isArray(e)?e:[];return(await Promise.all(n.map(async e=>{let n=ce(await u(e,t));return n?[n]:[]}))).flat()}function ue(e){let t=`homepage-card-title-${Math.random().toString(36).slice(2,9)}`,n=e.icon?`<i class="fal fa-lg fa-${c(e.icon)}" aria-hidden="true"></i>`:``,r=e.description?`<p class="homepage-card__description">${c(e.description)}</p>`:``,i=`
    <h3 class="homepage-card__title" id="${t}">${n}${c(e.title)}</h3>
    <div class="homepage-card__divider" aria-hidden="true"></div>
    ${r}`;return e.href?`<a class="homepage-card" href="${o(e.href)}" aria-labelledby="${t}">${i}</a>`:`<div class="homepage-card">${i}</div>`}function de(e){let{icon:t=``,label:n=``,href:r=``}=e||{},i=`
    <span class="mini-card__icon" aria-hidden="true">${t?`<i class="fa-thin fa-${c(t)}"></i>`:``}</span>
    <span class="mini-card__label">${c(n)}</span>`;return r?`<a class="mini-card" href="${o(r)}">${i}</a>`:`<div class="mini-card">${i}</div>`}async function fe(e,t,n){if(!e)return null;let r=e.metadata||{};return{title:p(r,`content-cardTitle`)||e.name||``,description:p(r,`content-cardDescription`),image:await f(e,t,n),date:p(r,`news-datePublished`),href:e.url||``}}function m(e){let t=Date.parse(e||``);return Number.isNaN(t)?-1/0:t}async function pe(e,t){let n=Array.isArray(e)?e:[];return(await Promise.all(n.map(async e=>{let n=await fe(await u(e,t),d(e),t);return n?[n]:[]}))).flat().sort((e,t)=>m(t.date)-m(e.date)).slice(0,x)}function me(e){let t=e.image,n=`news-card-title-${Math.random().toString(36).slice(2,9)}`,r=t?`
    <div class="card__image">
      <img src="${o(t.url)}" alt="${c(t.alt)}"${t.width?` width="${t.width}"`:``}${t.height?` height="${t.height}"`:``} />
    </div>`:``,i=e.description?`<p class="card__description">${c(e.description)}</p>`:``,a=`${r}
    <div class="card__content">
      <div class="news-card__header">
        <span class="news-card__icon" aria-hidden="true"><i class="fa-thin fa-${_}"></i></span>
        <span class="news-card__tag">${c(v)}</span>
        ${e.date?`<span class="news-card__date">${c(e.date)}</span>`:``}
      </div>
    ...
    </div>`;return e.href?`<a class="card" href="${o(e.href)}" aria-labelledby="${n}">${a}</a>`:`<article class="card">${a}</article>`}function he(e,t){return`
<div ${t} class="news-cards">
  <div class="news-cards__header">
    <h2 class="news-cards__title">${c(y)}</h2>
...
}`,...Q.parameters?.docs?.source}}},$=[`SinglePage`,`MultiplePages`,`Unresolvable`,`HomepageCards`,`MiniCards`,`MiniCardsNoLink`,`NewsCards`]}))();export{G as HomepageCards,J as MiniCards,X as MiniCardsNoLink,z as MultiplePages,Q as NewsCards,I as SinglePage,H as Unresolvable,$ as __namedExportsOrder,N as default};
//# sourceMappingURL=card.stories-9S2qemjH.js.map