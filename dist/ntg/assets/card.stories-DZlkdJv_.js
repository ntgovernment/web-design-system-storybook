import{n as e}from"./chunk-BVpWp9h4.js";import{n as t,t as n}from"./squizStoryRenderer-mGPdhelI.js";import{n as r,t as i}from"./squizInfo.mock-x2sHGyES.js";function a(e){return String(e??``).replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`).replace(/\"/g,`&quot;`).replace(/'/g,`&#039;`)}function o(e){if(!e)return null;let t=e.url||(Array.isArray(e.urls)?e.urls[0]:``)||``;return t?{url:t,alt:e.alt||e.name||``,width:e.width||null,height:e.height||null}:null}async function s(e,t){if(!e||typeof t?.resolveMatrixAssetById!=`function`)return null;let n;try{n=await t.resolveMatrixAssetById(e,[`url`,`urls`,`name`,`alt`,`witdh`,`height`])}catch(e){return console.error(e),null}return o(n.data)}function c(e,t){let n=e?.[t];return Array.isArray(n)&&n[0]||``}async function l(e,t){if(!e)return null;let n=e.metadata||{},r=c(n,`Image`);return{title:e.name||``,description:c(n,`Description`),image:await s(r,t),date:c(n,`Date`),tagLabel:c(n,`Category`),href:e.url||``,actionText:h}}function u(e){return Array.isArray(e?.include_dependents)}async function d(e,t){return!Array.isArray(e)||e.length===0||typeof t?.resolveUri!=`function`?[]:(await Promise.all(e.map(async e=>{if(!e)return[];let n;try{n=await t.resolveUri(e)}catch{return[]}if(!n)return[];if(u(n))return(await Promise.all(n.include_dependents.map(e=>l(e,t)))).filter(Boolean);let r=await l(n,t);return r?[r]:[]}))).flat()}function f(e){let t=e.image,n=`card-title-${Math.random().toString(36).slice(2,9)}`,r=t?`
    <div class="card__image">
      <img src="${a(t.url)}" alt="${a(t.alt)}"${t.width?` width="${t.width}"`:``}${t.height?` height="${t.height}"`:``} />
    </div>`:``,i=e.tagLabel||e.date?`
      <div class="card__header">
        ${e.tagLabel?`<span class="card__tag">${a(e.tagLabel)}</span>`:``}
        ${e.date?`<span class="card__date">${a(e.date)}</span>`:``}
      </div>`:``,o=e.description?`<p class="card__description">${a(e.description)}</p>`:``,s=e.actionText?`
      <div class="card__footer">
        <span class="card__action">${a(e.actionText)} <span class="card__action-icon">${m}</span></span>
      </div>`:``,c=`${r}
    <div class="card__content">${i}
      <div class="card__body">
        <h3 class="card__title" id="${n}">${a(e.title)}</h3>
        ${o}
      </div>${s}
    </div>`;return e.href?`<a class="card" href="${a(e.href)}" aria-labelledby="${n}">${c}</a>`:`<article class="card">${c}</article>`}function p(e){let{icon:t=``,label:n=``,href:r=``}=e||{},i=`
    <span class="mini-card__icon" aria-hidden="true">${t?`<i class="fa-thin fa-${a(t)}"></i>`:``}</span>
    <span class="mini-card__label" data-sq-field="label">${a(n)}</span>`;return r?`<a class="mini-card" href="${a(r)}">${i}</a>`:`<div class="mini-card">${i}</div>`}var m,h,g,_=e((()=>{m=`<svg viewBox="0 0 16 16" width="16" height="16" fill="none" aria-hidden="true" focusable="false"><path d="M3 8h10m0 0-4-4m4 4-4 4" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg>`,h=`Find out more`,g={async main(e={},t={}){e.debug&&delete t.debug;let{cardMode:n=`pages`,pageSources:r=[],miniCards:i=[]}=e;return n===`miniCards`?`
<div class="card-grid">
  ${(Array.isArray(i)?i:[]).map(p).join(``)}
</div>
`:`
<div class="card-grid">
  ${(await d(r,t.fns)).map(f).join(``)}
</div>
`}}}));function v({id:e,name:t,category:n,date:r,description:i,mockImage:a}){let o={Category:[n],Date:[r],Description:[i]},s;if(a){let n=`https://picsum.photos/id/${e}/640/480`;s=y+e,b[s]={url:n,urls:[`https://picsum.photos/id/${e}/640/480`],alt:`${t} thumbnail`},o.Image=[s]}return{id:String(e),type:`pageStandard`,name:t,url:`https://example.com/pages/${e}`,thumbnail:s?b[s]:null,metadata:o,child_count:0}}var y,b,x,S,C,w,T,E,D,O,k,A,j,M,N,P,F,I,L,R,z,B,V,H,U,W,G,K;await e((async()=>{n(),i(),_(),y=`mock-img-`,b={},x={"matrix-asset://ntg/1001":v({mockImage:!0,id:1015,name:`Community Consultation Opens for Major Project`,category:`News`,date:`17 Feb 2025`,description:`Residents invited to provide feedback on proposed infrastructure improvements in the greater Darwin area.`}),"matrix-asset://ntg/2001":{id:`2001`,type:`folder`,name:`News section`,url:`https://example.com/news`,child_count:3,include_dependents:[v({mockImage:!0,id:1016,name:`Annual Territory Day Celebrations`,category:`Event`,date:`15 Dec 2025`,description:`Join us for fireworks, live music, and family activities at Mindil Beach.`}),v({mockImage:!0,id:1018,name:`Understanding Territory Heritage Programs`,category:`News`,date:`28 Nov 2025`,description:`Explore how the NT Government is preserving indigenous heritage sites.`}),v({mockImage:!0,id:1019,name:`Road Closures This Weekend`,category:`Alert`,date:`3 Dec 2025`,description:`Temporary closures on the Stuart Highway for scheduled maintenance work.`})]},"matrix-asset://ntg/3001":{id:`3001`,type:`folder`,name:`Empty section`,url:`https://example.com/empty`,child_count:0,include_dependents:[]}},S=t(g.main),C=r({mock:{resolveUri(e){return x[e]??null},resolveMatrixAssetById(e,t=[]){return b[e]?{ok:!0,data:b[e]}:{ok:!0,data:null}}}}),w={title:`dxp/Card`},T={debug:!1,pageSources:[`matrix-asset://ntg/1001`]},E=await S.markup(T,C),D={render:e=>S(e,C),args:T,argTypes:{debug:{control:`boolean`},pageSources:{control:`object`,description:`Array of matrix-asset-uri strings, each an individually-picked page or a parent page whose children all become cards. Only these demo URIs resolve to anything in this Storybook preview (mocked): matrix-asset://ntg/1001, matrix-asset://ntg/2001, matrix-asset://ntg/3001.`}},parameters:{docs:{description:{story:`pageSources has one entry pointing directly at a single page -- one card, populated entirely from that page's own metadata (title, description, image, date, category, url). Hover/tab-focus the card to see the hover/focus states.`},source:{code:E,language:`html`,type:`code`}}}},O={debug:!0,pageSources:[`matrix-asset://ntg/2001`]},k=await S.markup(O,C),A={render:()=>S(O,C),parameters:{docs:{description:{story:`pageSources has one entry pointing at a parent/section page; every child page becomes its own card, arranged in the responsive grid (3 columns desktop, 2 tablet, 1 mobile).`},source:{code:k,language:`html`,type:`code`}}}},j={pageSources:[`matrix-asset://ntg/1001`,`matrix-asset://ntg/2001`]},M=await S.markup(j,C),N={render:()=>S(j,C),parameters:{docs:{description:{story:`pageSources combines one individually-picked page with one parent section -- the grid is the union of both (the individual page's card plus one card per child page).`},source:{code:M,language:`html`,type:`code`}}}},P={pageSources:[`matrix-asset://ntg/3001`]},F=await S.markup(P,C),I={render:()=>S(P,C),parameters:{docs:{description:{story:`A parent page with no children resolves to an empty grid, not an error.`},source:{code:F,language:`html`,type:`code`}}}},L={pageSources:[`matrix-asset://ntg/9999999`]},R=await S.markup(L,C),z={render:()=>S(L,C),parameters:{docs:{description:{story:`A source that fails to resolve at all contributes nothing, rendering an empty grid rather than throwing.`},source:{code:R,language:`html`,type:`code`}}}},B={cardMode:`miniCards`,miniCards:[{icon:`briefcase`,label:`Grant applicants`,href:`#`},{icon:`earth-americas`,label:`Service providers`,href:`#`},{icon:`circle-info`,label:`Get in touch`,href:`#`}]},V=await S.markup(B,C),H={render:()=>S(B,C),argTypes:{cardMode:{control:`radio`,options:[`pages`,`miniCards`]},miniCards:{control:`object`,description:`Array of { icon, label, href } -- icon is a Font Awesome classic/thin name without the "fa-" prefix. Only used when cardMode is "miniCards".`}},parameters:{docs:{description:{story:`cardMode "miniCards" switches the grid to manually-authored icon + label link items, in the same responsive 3/2/1-column layout as the page-derived "pages" mode. No resolveUri call happens in this mode.`},source:{code:V,language:`html`,type:`code`}}}},U={cardMode:`miniCards`,miniCards:[{icon:`briefcase`,label:`Grant applicants`,href:`#`},{icon:``,label:`No icon, no link`,href:``}]},W=await S.markup(U,C),G={render:()=>S(U,C),parameters:{docs:{description:{story:`An item with no href renders as a non-interactive div instead of a link; an item with no icon simply omits the icon markup -- both degrade gracefully rather than erroring.`},source:{code:W,language:`html`,type:`code`}}}},D.parameters={...D.parameters,docs:{...D.parameters?.docs,source:{originalSource:`{
  ...
  argTypes: {
    cardMode: {
      control: 'radio',
      options: ['pages', 'miniCards']
    },
    miniCards: {
      control: 'object',
      description: 'Array of { icon, label, href } -- icon is a Font Awesome classic/thin name without the "fa-" prefix. Only used when cardMode is "miniCards".'
    }
  },
  parameters: {
    docs: {
      description: {
        story: 'cardMode "miniCards" switches the grid to manually-authored icon + label link items, in the same responsive 3/2/1-column layout as the page-derived "pages" mode. No resolveUri call happens in this mode.'
      },
      source: {
        code: miniCardsHtml,
        language: 'html',
  ...
  parameters: {
    docs: {
      description: {
        story: 'An item with no href renders as a non-interactive div instead of a link; an item with no icon simply omits the icon markup -- both degrade gracefully rather than erroring.'
      },
      source: {
...
}`,...G.parameters?.docs?.source}}},K=[`SinglePage`,`ParentPage`,`Mixed`,`EmptySection`,`Unresolvable`,`MiniCards`,`MiniCardsNoLink`]}))();export{I as EmptySection,H as MiniCards,G as MiniCardsNoLink,N as Mixed,A as ParentPage,D as SinglePage,z as Unresolvable,K as __namedExportsOrder,w as default};
//# sourceMappingURL=card.stories-DZlkdJv_.js.map